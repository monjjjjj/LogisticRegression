...
def logreg_sgd(X, y, alpha = .001, epochs = 10000, eps=1e-4):
    ...
    for epoch in range(epochs):
        ...
        gradient = numpy.dot(X.T, (y_hat - numpy.array([y]).T))
        theta = theta - alpha * gradient
        if (numpy.absolute(alpha * gradient) < eps).all():
            print(numpy.absolute(alpha * g))
            break
    return theta
...
def plot_roc_curve(y_test, y_prob):
    ...
    for threshold in range(0, 101):
        threshold_func = lambda x : 1 if x > (threshold / 100) else 0
        y_threshold = list(map(threshold_func, y_prob))
        TN = len([x for x, y in zip(y_test, y_threshold) if x == y and x == 0])
        FN = len([x for x, y in zip(y_test, y_threshold) if x != y and x == 1])
        TP = len([x for x, y in zip(y_test, y_threshold) if x == y and x == 1])
        FP = len([x for x, y in zip(y_test, y_threshold) if x != y and x == 0])
